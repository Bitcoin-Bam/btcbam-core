#include <bitcoinbam/bitcoinbamDGP.h>
...
void BitcoinbamDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool BitcoinbamDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule BitcoinbamDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t BitcoinbamDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t BitcoinbamDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t BitcoinbamDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t BitcoinbamDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool BitcoinbamDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void BitcoinbamDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void BitcoinbamDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void BitcoinbamDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void BitcoinbamDGP::createParamsInstance(){
...
dev::Address BitcoinbamDGP::getAddressForBlock(unsigned int blockHeight){
...
void BitcoinbamDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BitcoinbamDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void BitcoinbamDGP::parseStorageOneUint64(uint64_t& value){
...
void BitcoinbamDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule BitcoinbamDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
