...
#include <bitcoinbam/bitcoinbamtransaction.h>
...
class BitcoinbamTransactionReceipt: public dev::eth::TransactionReceipt {
public:
...
struct ResultExecute{
    ...
    BitcoinbamTransactionReceipt txRec;
    CTransaction tx;
};

namespace bitcoinbam{
    template <class DB>
...
class BitcoinbamState : public dev::eth::State {
    
public:

    BitcoinbamState();

    BitcoinbamState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, BitcoinbamTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
    ...
    static const dev::Address createBitcoinbamAddress(dev::h256 hashTx, uint32_t voutNumber){
...
struct TemporaryState{
    std::unique_ptr<BitcoinbamState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<BitcoinbamState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(BitcoinbamState* _state, const std::vector<TransferInfo>& _transfers, const BitcoinbamTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const BitcoinbamTransaction& transaction;

    BitcoinbamState* state;
