...
#include <bitcoinbam/bitcoinbamstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

BitcoinbamState::BitcoinbamState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = BitcoinbamState::openDB(_path + "/bitcoinbamDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

BitcoinbamState::BitcoinbamState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute BitcoinbamState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BitcoinbamTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createBitcoinbamAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            bitcoinbam::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{
            ex,
            BitcoinbamTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs(), {}, {}),
        ...
        if (res.excepted == dev::eth::TransactionException::None) {
            ...
            return ResultExecute{
                res,
                BitcoinbamTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs(), {}, {}),
                tx ? *tx : CTransaction()
            };
        }
    }
...
std::unordered_map<dev::Address, Vin> BitcoinbamState::vins() const // temp
{
...
void BitcoinbamState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* BitcoinbamState::vin(dev::Address const& _a) const
{
    return const_cast<BitcoinbamState*>(this)->vin(_a);
}

Vin* BitcoinbamState::vin(dev::Address const& _addr)
{
...
// void BitcoinbamState::commit(CommitBehaviour _commitBehaviour)
// {
...
void BitcoinbamState::kill(dev::Address _addr)
{
...
void BitcoinbamState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void BitcoinbamState::deleteAccounts(std::set<dev::Address>& addrs){
...
void BitcoinbamState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void BitcoinbamState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void BitcoinbamState::validateTransfersWithChangeLog(){
...
void BitcoinbamState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!BitcoinbamState::addressInUse(delegationsAddress)){
        BitcoinbamState::createContract(delegationsAddress);
        BitcoinbamState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)});
