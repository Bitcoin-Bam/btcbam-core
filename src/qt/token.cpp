...
bool ToHash160(const std::string& strBitcoinbamAddress, std::string& strHash160)
{
    CTxDestination bitcoinbamAddress = DecodeDestination(strBitcoinbamAddress);
    if(!IsValidDestination(bitcoinbamAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&bitcoinbamAddress);
    if(keyid){
...
bool ToBitcoinbamAddress(const std::string& strHash160, std::string& strBitcoinbamAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    PKHash keyid(key);
    CTxDestination bitcoinbamAddress = keyid;
    if(IsValidDestination(bitcoinbamAddress)){
        strBitcoinbamAddress = EncodeDestination(bitcoinbamAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToBitcoinbamAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToBitcoinbamAddress(tokenEvent.receiver, tokenEvent.receiver);
