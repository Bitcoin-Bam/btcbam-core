...
BitcoinbamDelegation& GetBitcoinbamDelegation()
{
    static BitcoinbamDelegation bitcoinbamDelegation;
    return bitcoinbamDelegation;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, CValidationState& state, const CTransaction& tx, unsigned int nBits, uint32_t nTimeBlock, const std::vector<unsigned char>& vchPoD,  const COutPoint& headerPrevout, uint256& hashProofOfStake, uint256& targetProofOfStake, CCoinsViewCache& view)
{
    ...
    if (nHeight >= nOfflineStakeHeight && !Params().GetConsensus().delegationsAddress.IsNull())
    {
        ...
        BitcoinbamDelegation& bitcoinbamDelegation = GetBitcoinbamDelegation();
        if(!bitcoinbamDelegation.ExistDelegationContract())
        ...
        bool verifiedDelegation = bitcoinbamDelegation.VerifyDelegation(address, delegation);
...
bool GetDelegationFeeFromContract(const uint160& address, uint8_t& fee)
{
    Delegation delegation;
    BitcoinbamDelegation& bitcoinbamDelegation = GetBitcoinbamDelegation();
    bool ret = bitcoinbamDelegation.GetDelegation(address, delegation);
    if(ret) ret &= bitcoinbamDelegation.VerifyDelegation(address, delegation);
    if(ret)
    {
        fee = delegation.fee;
    }
