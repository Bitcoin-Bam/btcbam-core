...
void checkResult(bool isCreation, std::vector<BitcoinbamTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    BitcoinbamTxConverter converter(transaction, NULL);
    ExtractBitcoinbamTX bitcoinbamTx;
    BOOST_CHECK(converter.extractionBitcoinbamTransactions(bitcoinbamTx));
    std::vector<BitcoinbamTransaction> result = bitcoinbamTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(bitcoinbamtxconverter_tests, TestingSetup)
