...
extern std::unique_ptr<BitcoinbamState> globalState;
...
inline void initState(){
    ...
    const std::string dirBitcoinbam = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<BitcoinbamState>(new BitcoinbamState(dev::u256(0), BitcoinbamState::openDB(dirBitcoinbam, hashDB, dev::WithExisting::Trust), dirBitcoinbam + "/bitcoinbamDB", dev::eth::BaseState::Empty));
...
inline dev::Address createBitcoinbamAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline BitcoinbamTransaction createBitcoinbamTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    BitcoinbamTransaction txEth;
    if(recipient == dev::Address()){
        txEth = BitcoinbamTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = BitcoinbamTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<BitcoinbamTransaction> txs){
