...
void createTestContractsAndBlocks(TestChain100Setup* testChain100Setup, const valtype& code1, const valtype& code2, const valtype& code3, dev::Address addr){
    ...
    txs.push_back(createBitcoinbamTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, addr, 0));
    txs.push_back(createBitcoinbamTransaction(code1, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBitcoinbamTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    auto result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createBitcoinbamTransaction(code2, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBitcoinbamTransaction(code[4], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);

    generateBlocks(50);
    txs.clear();
    txs.push_back(createBitcoinbamTransaction(code3, 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBitcoinbamTransaction(code[6], 0, dev::u256(500000), dev::u256(1), ++hashTemp, addr, 0));
    result = executeBC(txs);
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test1){
    initState();
    contractLoading();
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    dev::eth::EVMSchedule schedule = bitcoinbamDGP.getGasSchedule(100);
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test2){
    ...
    contractLoading();
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    dev::eth::EVMSchedule schedule = bitcoinbamDGP.getGasSchedule(0);
    BOOST_CHECK(compareEVMSchedule(schedule, dev::eth::EIP158Schedule));
}
...
BOOST_AUTO_TEST_CASE(gas_schedule_default_state_test3){
    ...
    contractLoading();
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    dev::eth::EVMSchedule schedule = bitcoinbamDGP.getGasSchedule(1400);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<BitcoinbamTransaction> txs;
    txs.push_back(createBitcoinbamTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasScheduleDGP, 0));
    txs.push_back(createBitcoinbamTransaction(code[1], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBitcoinbamTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasScheduleDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(gas_schedule_one_paramsInstance_introductory_block_1_test2){
    ...
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    dev::eth::EVMSchedule schedule = bitcoinbamDGP.getGasSchedule(502); // After initializing the tests, the height of the chain 502
...
//    initState();
    ...
    for(size_t i = 0; i < 1300; i++){
        dev::eth::EVMSchedule schedule = bitcoinbamDGP.getGasSchedule(i);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test1){
    ...
    contractLoading();
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    uint32_t blockSize = bitcoinbamDGP.getBlockSize(100);
...
BOOST_AUTO_TEST_CASE(block_size_default_state_test2){
    ...
    contractLoading();
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    uint32_t blockSize = bitcoinbamDGP.getBlockSize(0);
    BOOST_CHECK(blockSize == DEFAULT_BLOCK_SIZE_DGP);
}
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<BitcoinbamTransaction> txs;
    txs.push_back(createBitcoinbamTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, BlockSizeDGP, 0));
    txs.push_back(createBitcoinbamTransaction(code[7], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBitcoinbamTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, BlockSizeDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(block_size_one_paramsInstance_introductory_block_1_test2){
    ...
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    uint32_t blockSize = bitcoinbamDGP.getBlockSize(502);
...
//    initState();
    ...
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint32_t blockSize = bitcoinbamDGP.getBlockSize(i);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test1){
    ...
    contractLoading();
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    uint64_t minGasPrice = bitcoinbamDGP.getMinGasPrice(100);
...
BOOST_AUTO_TEST_CASE(min_gas_price_default_state_test2){
    ...
    contractLoading();
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    uint64_t minGasPrice = bitcoinbamDGP.getMinGasPrice(0);
    BOOST_CHECK(minGasPrice == DEFAULT_MIN_GAS_PRICE_DGP);
}
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test1){
    ...
    dev::h256 hashTemp(hash);
    std::vector<BitcoinbamTransaction> txs;
    txs.push_back(createBitcoinbamTransaction(code[0], 0, dev::u256(500000), dev::u256(1), hashTemp, GasPriceDGP, 0));
    txs.push_back(createBitcoinbamTransaction(code[10], 0, dev::u256(500000), dev::u256(1), ++hashTemp, dev::Address(), 0));
    txs.push_back(createBitcoinbamTransaction(code[2], 0, dev::u256(500000), dev::u256(1), ++hashTemp, GasPriceDGP, 0));
    auto result = executeBC(txs);
...
BOOST_AUTO_TEST_CASE(min_gas_price_one_paramsInstance_introductory_block_1_test2){
    ...
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    uint64_t minGasPrice = bitcoinbamDGP.getMinGasPrice(502);
...
//    initState();
    ...
    BitcoinbamDGP bitcoinbamDGP(globalState.get());
    for(size_t i = 0; i < 1300; i++){
        uint64_t minGasPrice = bitcoinbamDGP.getMinGasPrice(i);
        std::function<bool(const uint64_t&, const uint64_t&)> func = compareUint64;
