 ...
 */
static const char* BITCOIN_PID_FILENAME = "bitcoinbamd.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    util::ThreadRename("bitcoinbam-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitcoinbamproject/bitcoinbam>";
    const std::string URL_WEBSITE = "<https://btcbam.com>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateBitcoinbam");
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.GetArg("-prune", 0)) {
        if (gArgs.GetBoolArg("-txindex", DEFAULT_TXINDEX))
		    InitWarning(_("Ignoring Prune Mode: Incompatible with -txindex.").translated);
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path bitcoinbamStateDir = GetDataDir() / "stateBitcoinbam";
                bool fStatus = fs::exists(bitcoinbamStateDir);
                const std::string dirBitcoinbam(bitcoinbamStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsBitcoinbamstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<BitcoinbamState>(new BitcoinbamState(dev::u256(0), BitcoinbamState::openDB(dirBitcoinbam, hashDB, dev::WithExisting::Trust), dirBitcoinbam, existsBitcoinbamstate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(bitcoinbamStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setBitcoinbamSchedule(bitcoinbamDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
