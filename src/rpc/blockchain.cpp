...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // bitcoinbam
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // bitcoinbam

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // bitcoinbam
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // bitcoinbam
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // bitcoinbam
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // bitcoinbam
...
UniValue transactionReceiptToJSON(const BitcoinbamTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination bitcoinbamSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(bitcoinbamSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&bitcoinbamSenderAddress);
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address string"},
                },
                RPCResult{
            "{\n"
    ...
    if(!bitcoinbamDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = bitcoinbamDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address string for staker"},
                },
                RPCResult{
            "[{\n"
    ...
    if(!bitcoinbamDelegation.FilterDelegationEvents(events, filter)) {
