...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setBitcoinbamSchedule(bitcoinbamDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = bitcoinbamDGP.getBlockSize(nHeight);
    minGasPrice = bitcoinbamDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = bitcoinbamDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    BitcoinbamTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractBitcoinbamTX resultConverter;
    if(!convert.extractionBitcoinbamTransactions(resultConverter)){
    ...
    dev::u256 txGas = 0;
    for(BitcoinbamTransaction bitcoinbamTransaction : bitcoinbamTransactions){
        txGas += bitcoinbamTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + bitcoinbamTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(bitcoinbamTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, bitcoinbamTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= nCheckpointSpan)
        {
            ...
            bitcoinbamDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = bitcoinbamDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                bitcoinbamDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                bitcoinbamDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            bitcoinbamDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            bitcoinbamDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    BitcoinbamDelegation bitcoinbamDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= nCheckpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                bitcoinbamDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = bitcoinbamDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    bitcoinbamDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    bitcoinbamDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                bitcoinbamDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(bitcoinbamDelegations.GetDelegation(address, delegation) && BitcoinbamDelegation::VerifyDelegation(address, delegation))
                    {
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "bitcoinbamstake";
...
void StakeBitcoinbams(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
