    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"setlabel",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address to be associated with a label."},
...
static UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"sendtoaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address to send to."},
            ...
            "                             dirty if they have previously been used in a transaction."},
                    {"senderaddress", RPCArg::Type::STR, RPCArg::Optional::OMITTED_NAMED_ARG, "The bitcoinbam address that will be used to send money from."},
    ...
    senderAddress = DecodeDestination(request.params[9].get_str());
        if (!IsValidDestination(senderAddress))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Bitcoinbam address to send from");
        else
            fHasSender=true;
    }
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"splitutxosforaddress",
                "\nSplit an address coins into utxo between min and max value." +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address to split utxos."},
...
static UniValue createcontract(const JSONRPCRequest& request){
    ...
    uint64_t blockGasLimit = bitcoinbamDGP.getBlockGasLimit(::ChainActive().Height());
    uint64_t minGasPrice = CAmount(bitcoinbamDGP.getMinGasPrice(::ChainActive().Height()));
    CAmount nGasPrice = (minGasPrice>DEFAULT_GAS_PRICE)?minGasPrice:DEFAULT_GAS_PRICE;
                ...
                HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"gasPrice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice BTCBAM price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                    {"senderaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED, "The bitcoinbam address that will be used to create the contract."},
    ...
    CAmount nGasPrice = (minGasPrice>DEFAULT_GAS_PRICE)?minGasPrice:DEFAULT_GAS_PRICE;
                ...
                RPCHelpMan{"sendtocontract",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                    {
                        ...
                        {"gasPrice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice Bitcoinbam price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                        {"senderaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED, "The bitcoinbam address that will be used as sender."},
                        {"broadcast", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED, "Whether to broadcast the transaction or not."},
                        {"changeToSender", RPCArg::Type::BOOL, RPCArg::Optional::OMITTED, "Return the change to the sender."},
                    },
                    RPCResult{
...
static UniValue removedelegationforaddress(const JSONRPCRequest& request){
    ...
    CAmount nGasPrice = (minGasPrice>DEFAULT_GAS_PRICE)?minGasPrice:DEFAULT_GAS_PRICE;
                ...
                RPCHelpMan{"removedelegationforaddress",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                    {
                        {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address to remove delegation, the address will be used as sender too."},
                        {"gasLimit", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasLimit, default: "+i64tostr(DEFAULT_GAS_LIMIT_OP_SEND)+", max: "+i64tostr(blockGasLimit)},
                        {"gasPrice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice Bitcoinbam price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                    },
                    RPCResult{
    ...
    UniValue datahex = BitcoinbamDelegation::BytecodeRemove();
    UniValue amount = 0;
...
static UniValue setdelegateforaddress(const JSONRPCRequest& request){
    ...
    CAmount nGasPrice = (minGasPrice>DEFAULT_GAS_PRICE)?minGasPrice:DEFAULT_GAS_PRICE;
                ...
                RPCHelpMan{"setdelegateforaddress",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                    {
                        {"staker", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address for the staker."},
                        {"fee", RPCArg::Type::NUM, RPCArg::Optional::NO, "Percentage of the reward that will be paid to the staker."},
                        {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address that contain the coins that will be delegated to the staker, the address will be used as sender too."},
                        {"gasLimit", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasLimit, default: "+i64tostr(DEFAULT_GAS_LIMIT_OP_CREATE)+", max: "+i64tostr(blockGasLimit)},
                        {"gasPrice", RPCArg::Type::AMOUNT, RPCArg::Optional::OMITTED, "gasPrice Bitcoinbam price per gas unit, default: "+FormatMoney(nGasPrice)+", min:"+FormatMoney(minGasPrice)},
                    },
                    RPCResult{
    ...
    if(!BitcoinbamDelegation::BytecodeAdd(hexStaker, fee, PoD, datahex, errorMessage))
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"signmessage",
                "\nSign a message with the private key of an address" +
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address to use for the private key."},
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getreceivedbyaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address for transactions."},
    ...
    RPCHelpMan{"sendmany",
                    ...
                    HelpRequiringPassphrase(pwallet) + "\n",
                {
                    ...
                    {"amounts", RPCArg::Type::OBJ, RPCArg::Optional::NO, "A json object with addresses and amounts",
                        {
                            {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "The bitcoinbam address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value"},
                        },
                    },
        ...
        return NullUniValue;
            ...
            RPCHelpMan{"addmultisigaddress",
                ...
                "If 'label' is specified, assign address to that label.\n",
                {
                    ...
                    {"keys", RPCArg::Type::ARR, RPCArg::Optional::NO, "A json array of bitcoinbam addresses or hex-encoded public keys",
                        {
                            {"key", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "bitcoinbam address or hex-encoded public key"},
                        },
                        },
...
static UniValue gettransaction(const JSONRPCRequest& request_)
{
    ...
    if (!shouldWaitConf) {
        ...
        if(!request.req)
            throw JSONRPCError(RPC_INTERNAL_ERROR, "No HTTP connection. Waitconf is available from bitcoinbam-cli, not bitcoinbam-qt");
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"walletpassphrase",
                ...
                "This is needed prior to performing transactions related to private keys such as sending BTCBAM and staking\n"
            "\nNote:\n"
...
static UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    RPCHelpMan{
                ...
                "Optionally filter to only include txouts paid to specified addresses.\n",
                {
                    ...
                    {"addresses", RPCArg::Type::ARR, /* default */ "empty array", "A json array of bitcoinbam addresses to filter",
                        {
                            {"address", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "bitcoinbam address"},
                        },
                    },
    ...
    RPCHelpMan{"fundrawtransaction",
                ...
                "Only pay-to-pubkey, multisig, and P2SH versions thereof are currently supported for watch-only\n",
                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR, /* default */ "pool address", "The bitcoinbam address to receive the change"},
    ...
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
            ...
            RPCHelpMan{"getaddressinfo",
                ...
                "to be in the wallet.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The bitcoinbam address to get the information of."},
                },
                RPCResult{
            "{\n"
                ...
                "Implements the Creator and Updater roles.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "a json array with outputs (key-value pairs), where none of the keys are duplicated.\n"
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the bitcoinbam address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                    ...
                    {"options", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED_NAMED_ARG, "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX, /* default */ "pool address", "The bitcoinbam address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM, /* default */ "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR, /* default */ "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL, /* default */ "true for watch-only wallets, otherwise false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL, /* default */ "false", "Lock selected unspent outputs"},
