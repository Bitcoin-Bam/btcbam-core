...
std::unique_ptr<BitcoinbamState> globalState;
...
bool fAddressIndex = false; // bitcoinbam
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        BitcoinbamDGP bitcoinbamDGP(globalState.get(), fGettingValuesDGP);
        uint64_t minGasPrice = bitcoinbamDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = bitcoinbamDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        BitcoinbamTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractBitcoinbamTX resultConverter;
        if(!converter.extractionBitcoinbamTransactions(resultConverter)){
        ...
        std::vector<BitcoinbamTransaction> bitcoinbamTransactions = resultConverter.first;
        std::vector<EthTransactionParams> bitcoinbamETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(const BitcoinbamTransaction& bitcoinbamTransaction : bitcoinbamTransactions){
            sumGas += bitcoinbamTransaction.gas() * bitcoinbamTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, bitcoinbamTransaction.gasPrice());
            } else {
                txMinGasPrice = bitcoinbamTransaction.gasPrice();
            }
            VersionVM v = bitcoinbamTransaction.getVersion();
            if(v.format!=0)
            ...
            if(bitcoinbamTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(bitcoinbamTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(bitcoinbamTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += bitcoinbamTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)bitcoinbamTransaction.gasPrice() < minGasPrice)
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    if (nHeight == 1) {
        ...
        for (int i = consensusParams.rewardEpoch; i <= nHeight; i += consensusParams.rewardEpoch) {
            if (i >= consensusParams.rewardEpoch * 5)
                break;
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // bitcoinbam
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // bitcoinbam
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, CBlockIndex* pblockindex, const dev::Address& sender, uint64_t gasLimit) {
    ...
    BitcoinbamDGP qtumDGP(globalState.get(), fGettingValuesDGP);
    ...
    BitcoinbamTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<BitcoinbamTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path bitcoinbamDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(bitcoinbamDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(bitcoinbamDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(BitcoinbamTransaction& tx : txs){
        //validate VM version
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{
                execRes,
                BitcoinbamTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries(), {}, {}),
                CTransaction()
            });
            continue;
        }
...
bool BitcoinbamTxConverter::extractionBitcoinbamTransactions(ExtractBitcoinbamTX& bitcoinbamtx){
    ...
    std::vector<BitcoinbamTransaction> resultTX;
    ...
    bitcoinbamtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool BitcoinbamTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool BitcoinbamTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
BitcoinbamTransaction BitcoinbamTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    BitcoinbamTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = BitcoinbamTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = BitcoinbamTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t BitcoinbamTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // bitcoinbam
    BitcoinbamDGP bitcoinbamDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setBitcoinbamSchedule(bitcoinbamDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = bitcoinbamDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = bitcoinbamDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = bitcoinbamDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // bitcoinbam
...
///////////////////////////////////////////////////////////////////////////////////////// bitcoinbam
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            BitcoinbamTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractBitcoinbamTX resultConvertBitcoinbamTX;
            if(!convert.extractionBitcoinbamTransactions(resultConvertBitcoinbamTX)){
            ...
            if(!CheckMinGasPrice(resultConvertBitcoinbamTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertBitcoinbamTX.first, blockGasLimit, pindex->pprev);
            ...
            for(BitcoinbamTransaction& qtx : resultConvertBitcoinbamTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertBitcoinbamTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertBitcoinbamTX.first[k].getNVout(),
                        resultConvertBitcoinbamTX.first[k].from(),
                        resultConvertBitcoinbamTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // bitcoinbam
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // bitcoinbam
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // bitcoinbam
            globalState->setRootUTXO(oldHashUTXORoot); // bitcoinbam
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // bitcoinbam
        uint32_t sizeBlockDGP = bitcoinbamDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Bitcoinbam PoS we need to sync this after every block to ensure txdb is populated for
