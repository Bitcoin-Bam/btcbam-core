Gitian building
================
*Setup instructions for a Gitian build of Bitcoinbam Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitcoinbam
...
- [Building Bitcoinbam Core](#building-bitcoinbam-core)
...
Clone the git repositories for bitcoinbam and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitcoinbamproject/bitcoinbam --recursive
git clone https://github.com/bitcoinbamproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/bitcoinbam/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the bitcoinbam repository under 'Fetch and create inputs' to install sources which require
...
To build Bitcoinbam Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitcoinbam repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinbam/.git/
...
For example:
```bash
URL=https://github.com/laanwj/bitcoinbam.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoinbam=${COMMIT} --url bitcoinbam=${URL} ../bitcoinbam/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinbam=${COMMIT} --url bitcoinbam=${URL} ../bitcoinbam/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinbam=${COMMIT} --url bitcoinbam=${URL} ../bitcoinbam/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-bitcoinbam:
./bin/gbuild --commit bitcoinbam=${COMMIT},cpp-eth-bitcoinbam=${ETHCOMMIT} --url bitcoinbam=${URL},cpp-eth-bitcoinbam=${ETHURL} ../bitcoinbam/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the bitcoinbam git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcoinbam/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/bitcoinbamproject/bitcoinbam-detached-sigs.git

BTCPATH=/some/root/path/bitcoinbam
SIGPATH=/some/root/path/bitcoinbam-detached-sigs

./bin/gbuild --url bitcoinbam=${BTCPATH},signature=${SIGPATH} ../bitcoinbam/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinbam-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinbam-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoinbam-osx-build.assert
```
...
[bitcoinbamproject/gitian.sigs](https://github.com/bitcoinbamproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@www.btcbam.com) and he will commit them.
